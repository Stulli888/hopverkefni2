(function () {
  ...
  function formatDate(created) {
    var s = "string";
    var now = Date.now();
    var difference = now - created; // Tími síðan myndband var búið til í klukkustundum

    var elapsed = parseInt(difference / 1000 / 60 / 60);

    if (elapsed > 8760) {
      var years = parseInt(elapsed / 8760);

      if (years > 1) {
        s = "Fyrir ".concat(years, " \xE1rum s\xED\xF0an");
      } else {
        s = "Fyrir ".concat(years, " \xE1ri s\xED\xF0an");
      }
    } else if (elapsed > 720) {
      var months = parseInt(elapsed / 720);

      if (months > 1) {
        s = "Fyrir ".concat(months, " m\xE1nu\xF0um s\xED\xF0an");
      } else {
        s = "Fyrir ".concat(months, " m\xE1nu\xF0i s\xED\xF0an");
      }
    } else if (elapsed > 168) {
      var weeks = parseInt(elapsed / 168);

      if (weeks > 1) {
        s = "Fyrir ".concat(weeks, " vikum s\xED\xF0an");
      } else {
        s = "Fyrir ".concat(weeks, " viku s\xED\xF0an");
      }
    } else if (elapsed > 24) {
      var days = parseInt(elapsed / 24);

      if (days > 1) {
        s = "Fyrir ".concat(days, " d\xF6gum s\xED\xF0an");
      } else {
        s = "Fyrir ".concat(days, " degi s\xED\xF0an");
      }
    } else {
      if (elapsed > 1) {
        s = "Fyrir ".concat(elapsed, " klukkustundum s\xED\xF0an");
      } else {
        s = "Fyrir ".concat(elapsed, " klukkustund s\xED\xF0an");
      }
    }
    ...
    _createClass(VideoList, [{
      ...
      value: function showList(data) {
        ...
        data.categories.forEach(function (cat) {
          ...
          var vidBoxes = cat.videos.map(function (vidBox) {
            var _col$classList;
